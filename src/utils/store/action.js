...
export const updateSongInfo = (songInfo) => {
    ...
    if (info.platform === 'local') {
      ...
      if (info.textInfo) {
        ...
        if (allSongs[textInfo.aId]) {
          ...
          getSongInfo(textInfo.aId, info.localPath).then(([realId]) => {
            allSongs[aId] = {
              ...allSongs[aId],
              ...allSongs[realId],
            };
  ...
  needUpdateList &&
    (playingList.trueList = playingList.raw.filter(
      (aId) => allSongs[aId].url || allSongs[aId].localPath,
    ));
};
...
export const findMusic = {
  ...
  push(aId) {
    ...
    if (this.num < 2 && quene.length) {
      ...
      song.noUrl = true;
      const key = `${song.name.replace(/\(|\)|（|）/g, ' ')} ${song.ar
        .map((a) => a.name)
        .join(' ')}`;
...
export const handleSongs = (list) => {
  ...
  list.forEach((s) => {
    ...
    allSongs[s.aId].localPath &&
      !allSongs[s.aId].checkedFile &&
      ipcRenderer.send('LOAD_LOCAL_SINGLE_FILE', s.aId);
    !allSongs[s.aId].url &&
      !allSongs[s.aId].noUrl &&
      !s.localPath &&
      s.aId &&
      getUrlArr.push(s.aId);
  });
...
export const handlePlayLists = (list) => {
  ...
  return list.map((p) => {
    ...
    allList[p.aId].list = (allList[p.aId].list || []).map((s) =>
      typeof s === 'string' ? s : s.aId,
    );
    return p.aId;
  });
};
...
export const search = async ({ keyword: key, type, pageNo, pageSize }) => {
  ...
  searchInfo.result[type] = [
    ...(searchInfo.pageNo === 1 ? [] : searchInfo.result[type]),
    ...newList,
  ];
...
export const getUserList = async ({ id, platform } = {}) => {
  ...
  if (uId === user[_p].id) {
    ...
    data.forEach(({ creator: { id }, aId }) => {
      `${id}` === `${uId}` ? (user[_p].myList[aId] = 1) : (user[_p].subList[aId] = 1);
    });
  }
...
export const getQQLoginStatus = async (c) => {
  ...
  const {
    data: { creator },
  } = await request({ api: 'QQ_USER_DETAIL', data: { id: user.qq.id } });
  user.qq = {
    ...user.qq,
    ...creator,
...
export const getDaily = async (platform) =>
  request({ api: 'DAILY_PLAYLIST', data: { ownCookie: 1 } }, platform);

// 第一次登录的时候调用
export const initLogin = () =>
  Promise.all([get163LoginStatus(), getQQLoginStatus()]).then((loginRes) => {
...
export const getLyric = async (aId) => {
  ...
  const {
    data: { lyric, trans },
  } = await request({ api: 'LYRIC', data: { id: s.id, _p: s.platform } });
  let lyricObj = {};
  lyric && handleLyric(lyric, 'str', lyricObj);
...
export const downReq = async (info) => {
  ...
  let { data } = await axios({
    ...
    onDownloadProgress: ({ loaded, total }) =>
      (downloadList[dId].progress = ((loaded / total) * 100).toFixed(2) / 1),
  }).catch((err) => {
...
export const addSong2Playlist = async ({ aId, pId, type, toast = true }) => {
  ...
  switch (s.platform) {
    case 'qq':
      l = Object.values(allList).find(
        ({ dirid, userId }) => userId === user.qq.id && dirid === pId,
      );
      if (pId === 201) {
  ...
  if (l) {
    type ? l.list.unshift(aId) : (l.list = l.list.filter((v) => v !== aId));
...
export const mixSongHandle = {
  ...
  removeFromPlayinig(list) {
    window.event.stopPropagation();
    updatePlayingList(
      window.$state.playingList.raw.filter((id) => id.indexOf(list) === -1),
      true,
    );
  },
  likeMusic,
...
export const handlePerformanceMode = (mode) => {
  ...
  if (
    ...
    mode &&
    document.getElementById('m-player')
  ) {
...
export const handleVolume = (start, end, operation) => {
  ...
  const delta = Math.max(0.01, abs / 20) * ((end - start) / abs);
  window.FADE_VOLUME = {
    start,
    end,
    operation,
...
export const getMoreRadioList = async (force) => {
  ...
  force ? updatePlaying(list[0], list, true, radioId) : updatePlayingList(list);
};
